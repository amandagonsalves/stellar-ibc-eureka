...
pub async fn run(
    ...
    if !wasm_checksum_present(&cfg.hermes_config) {
        bail!(
            "wasm_checksum_hex is empty in {} — upload the light client first: interstellar contracts upload-wasm",
            cfg.hermes_config
        );
    }
    ...
    logger::hint("next: interstellar clients counterparty stellar / cosmos");
