...
pub async fn run(root: &Path, cfg: &OpsConfig, http: &reqwest::Client) -> Result<()> {
    ...
    shared::check("docker", run::has("docker"), "required to run the chain + services");
    shared::check("stellar", run::has("stellar"), "Soroban CLI, used to build/deploy contracts");
    shared::check("cargo", run::has("cargo"), "builds the wasm light client + this CLI");
